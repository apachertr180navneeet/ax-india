...
@section('content')
    <div class="row">
        <div class="col-lg-8">
            ...
            <div class="d-flex align-items-center justify-content-between p-3 rounded-4 mb-3" style="background-color: var(--yt-dark-card); border: 1px solid var(--yt-border);">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <a href="{{ route('channel', $video->user_id) }}" class="text-decoration-none fw-semibold text-light">{{ $video->user?->full_name ?? 'Unknown' }}</a>
                        <p class="mb-0 text-muted small">{{ number_format($video->user?->profile?->subscribers_count ?? 0) }} subscribers</p>
                    </div>
                </div>
                @auth
                    <button class="btn btn-danger btn-sm subscribe-btn px-3 rounded-pill fw-semibold" data-creator-id="{{ $video->user_id }}">
                        {{ $isSubscribed ?? false ? 'Subscribed' : 'Subscribe' }}
                    </button>
                @endauth
            </div>
            <div class="p-3 rounded-4 mb-4" style="background-color: var(--yt-dark-card); border: 1px solid var(--yt-border);">
                <p class="mb-1 text-break text-light">{{ $video->description ?? 'No description.' }}</p>
            </div>
            <h6 class="fw-semibold mb-3 text-light">{{ count($comments) }} Comments</h6>
            @auth
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="mt-2 text-end">
                                <button type="submit" class="btn btn-danger btn-sm rounded-pill px-3">Comment</button>
                            </div>
                        </div>
                    </div>
                </form>
            @else
                <p class="text-muted mb-4"><a href="{{ route('login') }}" class="text-danger">Sign in</a> to add a comment.</p>
            @endauth
            @foreach ($comments as $comment)
                <div class="d-flex mb-3">
                    ...
                    <div>
                        <p class="mb-0 small fw-semibold text-light">{{ $comment->user?->full_name ?? 'Unknown' }} <span class="text-muted fw-normal ms-2">{{ $comment->created_at?->diffForHumans() }}</span></p>
        ...
        <div class="col-lg-4">
            <h6 class="fw-semibold mb-3 text-light">Related Videos</h6>
            @forelse ($relatedVideos as $related)
                <div class="d-flex mb-3">
                    ...
                    <div class="flex-grow-1 min-width-0">
                        <a href="{{ route('watch', $related->slug) }}" class="text-decoration-none text-light fw-semibold small" style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">{{ $related->title }}</a>
